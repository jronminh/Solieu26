...
def test_score_tong_luong_may_hit_within_window_tolerance():
    ...
    assert score_tong_luong_may(2, {"tong_luong_may": 5}) is True


def test_score_tong_luong_may_miss_outside_window_tolerance():
    assert score_tong_luong_may(0, {"tong_luong_may": 5}) is False


def test_score_tong_luong_may_clamps_at_edge_windows():
    assert score_tong_luong_may(8, {"tong_luong_may": 5}) is False


def test_score_tong_luong_may_none_forecast_or_obs():
    assert score_tong_luong_may(None, {"tong_luong_may": 5}) is None
    assert score_tong_luong_may(2, {"tong_luong_may": None}) is None


def test_score_tong_luong_may_obscured_sky_is_na():
    assert score_tong_luong_may(2, {"tong_luong_may": "/"}) is None
...
def test_score_do_cao_man_may_no_ceiling_matches_top_bucket():
    assert score_do_cao_man_may(14, {"do_cao_man_may": NO_CEILING}) is True


def test_score_do_cao_man_may_no_ceiling_within_tolerance_of_neighbour():
    assert score_do_cao_man_may(13, {"do_cao_man_may": NO_CEILING}) is True


def test_score_do_cao_man_may_no_ceiling_outside_tolerance():
    assert score_do_cao_man_may(12, {"do_cao_man_may": NO_CEILING}) is False
...
def test_score_do_cao_man_may_bound_value_falls_in_upper_bucket():
    ...
    assert score_do_cao_man_may(1, {"do_cao_man_may": 50}) is True
    assert score_do_cao_man_may(3, {"do_cao_man_may": 50}) is False  # off by 2, outside tolerance


def test_score_do_cao_man_may_missing_data_is_none():
    assert score_do_cao_man_may(1, {"do_cao_man_may": None}) is None
    assert score_do_cao_man_may(None, {"do_cao_man_may": 50}) is None
...
def test_score_tam_nhin_bound_value_falls_in_upper_bucket():
    assert score_tam_nhin(1, {"tam_nhin": 0.5}) is True
    assert score_tam_nhin(3, {"tam_nhin": 0.5}) is False


def test_score_tam_nhin_missing_data_is_none():
    assert score_tam_nhin(1, {"tam_nhin": None}) is None
    assert score_tam_nhin(None, {"tam_nhin": 5}) is None
...
def test_score_toc_do_gio_hit_within_window_tolerance():
    assert score_toc_do_gio(2, {"toc_do_gio": 5}) is True
...
def test_score_toc_do_gio_above_no_speed_min_is_none():
    ...
    assert score_toc_do_gio(0, {"toc_do_gio": 16}) is None


def test_score_toc_do_gio_missing_data_is_none():
    assert score_toc_do_gio(2, {"toc_do_gio": None}) is None
    assert score_toc_do_gio(None, {"toc_do_gio": 5}) is None
...
def test_score_huong_gio_wraps_around_compass():
    ...
    assert score_huong_gio(0, {"huong_gio": 15, "toc_do_gio": 5}) is True


def test_score_huong_gio_accepts_string_label_for_obs_dir():
    n_idx = LABELS.index("N")
    assert score_huong_gio(n_idx, {"huong_gio": "NNE", "toc_do_gio": 5}) is True
...
def test_score_huong_gio_calm_wind_drops_direction():
    ...
    assert score_huong_gio(0, {"huong_gio": 0, "toc_do_gio": 2}) is None


def test_score_huong_gio_missing_data_is_none():
    assert score_huong_gio(0, {"huong_gio": None, "toc_do_gio": 5}) is None
    assert score_huong_gio(None, {"huong_gio": 0, "toc_do_gio": 5}) is None
    assert score_huong_gio(0, {"huong_gio": 0, "toc_do_gio": None}) is None
...
def test_score_hien_tuong_mega_mismatch_is_false():
    assert score_hien_tuong("N_0", "dem", {"hien_tuong": "suong_mu", "hour": 1}) is False


def test_score_hien_tuong_mega_match_sub_within_tolerance():
    # hour 1 -> 'dem'; forecast sub 'dem' -> exact match
    assert score_hien_tuong("N_0", "dem", {"hien_tuong": "N_0", "hour": 1}) is True


def test_score_hien_tuong_mega_match_sub_outside_tolerance():
    # hour 1 -> 'dem' (idx 1); forecast sub 'chieu' (idx 4) -> off by 3, no wrap
    assert score_hien_tuong("N_0", "chieu", {"hien_tuong": "N_0", "hour": 1}) is False


def test_score_hien_tuong_missing_data_is_none():
    assert score_hien_tuong(None, "dem", {"hien_tuong": "N_0", "hour": 1}) is None
    assert score_hien_tuong("N_0", "dem", {"hien_tuong": None, "hour": 1}) is None
